...
zig_build () {
    ...
    cp "../target/$TARGET/release/$LIBNAME" "$LIBRARY_PATH/linux/"
}
...
win_build () {
    ...
    mkdir "$PLATFORM_NAME"
    cp "../target/$TARGET/release/$LIBNAME" "$PLATFORM_NAME/"
    cp "../target/$TARGET/release/$LIBNAME" "$LIBRARY_PATH/windows/"
}

# Build all the dynamic libraries
LINUX_LIBNAME=libdr_libphonenumber.so
zig_build aarch64-unknown-linux-gnu linux-arm64 $LINUX_LIBNAME
zig_build x86_64-unknown-linux-gnu linux-x64 $LINUX_LIBNAME
WINDOWS_LIBNAME=dr_libphonenumber.dll
